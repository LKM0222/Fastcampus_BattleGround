...
public class BlinkHUD : MonoBehaviour
{
    ...
    private void Update() 
    {
        if(blink)
        {
            colorGradingLayer.saturation.value = Mathf.Lerp(
                0, dyingSaturation, colorGradeTimer / blinkFadeStep);
            colorGradingLayer.brightness.value = Mathf.Lerp(
                0, dyingBrightness, colorGradeTimer / blinkFadeStep);
